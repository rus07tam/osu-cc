...
namespace osucc.UI.Plugins
{
    ...
    /// description, tags, load status), with a search box and clickable tag chips that filter the
    /// list by name, author, id or tag. Opened from the Specials settings section.
    /// </summary>
    public partial class PluginsOverlay : OsuCcShearedOverlay
    {
        ...
        private void load()
        {
            ...
            searchBox.Current.BindValueChanged(e => applyFilter(e.NewValue), true);
        }

        /// <summary>Sets the search filter (also used by tag chips) to the given text and shows the overlay.</summary>
        public void SetFilter(string? filter)
        {
            searchBox.Current.Value = filter ?? string.Empty;
            Show();
        }
        ...
        private static bool matchesFilter(PluginEntry entry, string query)
        {
            ...
            return haystacks.Any(s => s.Contains(query, StringComparison.OrdinalIgnoreCase));
        }
        ...
        protected override void LoadComplete()
        {
            ...
            foreach (var entry in entries)
                cards.Add(new PluginCard(entry, moveCard, deletePlugin, tag => searchBox.Current.Value = tag));
        ...
        private sealed partial class PluginCard : Container
        {
                ...
                Action<string> tagSelected)
            {
                ...
                var header = createHeader(entry, moveRequested, deleteRequested, tagSelected);
            ...
            private GridContainer createHeader(PluginEntry entry, Action<PluginCard, int> moveRequested, Action<PluginCard> deleteRequested, Action<string> tagSelected)
            {
                ...
                var meta = new FillFlowContainer
                {
                    ...
                    Children = new Drawable[]
                    {
                        ...
                        new Container
                        {
                            ...
                            Child = PluginCardLayout.CreateTagsValue(entry, tagSelected, fontSize: 11, maxShown: 3),
                        },
                    },
                };
